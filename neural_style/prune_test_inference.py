...
def stylize(args):
    ...
    if args.prune_percent:
        ...
        print("Creating dependency graph with the content image as the dummy input...")
        DG = tp.DependencyGraph().build_dependency(style_model, example_inputs=content_image)

        print("Pruning the conv3 layer...")
        prune_layer = style_model.conv3.conv2d

        # pruning_idxs = tp.strategy.L1Strategy().get_pruning_plan(prune_layer.weight, amount=prune_percent)
        # pruning_plan = DG.get_pruning_plan(prune_layer, tp.prune_conv_out_channels, idxs=pruning_idxs)

        strategy = tp.strategy.L1Strategy()  # Use the L1 norm to rank filters
        prune_idxs = strategy(prune_layer.weight, amount=args.prune_percent)
        pruning_plan = DG.get_pruning_plan(prune_layer, tp.prune_conv_out_channels, idxs=prune_idxs)

        print(f"Pruning Plan: {pruning_plan}")
        pruning_plan.exec()
        ...
        # print("Pruning the conv2 layer...")
        # conv_layer = style_model.conv2.conv2d
        # # Prune 30% of the filters by L1 norm, removing entire filters along dim=0
        # amount_to_prune = args.prune_percent
        # prune.ln_structured(
        #     conv_layer,
        ...
        # print("Pruning the deconv2 layer...")
        # conv_layer = style_model.deconv2.conv2d
        # # Prune 30% of the filters by L1 norm, removing entire filters along dim=0
        # amount_to_prune = args.prune_percent
        # prune.ln_structured(
        #     conv_layer,
